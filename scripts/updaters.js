...
const generateBuyTx = async (
  ...
  if (1 == PARAMS.maxBuySafe && 0 == _realBuyMethod) {
    ...
    const amountIn = ethers.utils.parseEther(PARAMS.Max_Weth_Spend.toString());
    ...
    if (_tokenBuyAmount && _tokenBuyAmount.gt(ao)) {
      console.log(
        "TokenbuyAmount cost exceeds Max_Weth_Spend Standing down!!!"
      );
      //_tokenBuyAmount = ao;
      return states;
    }
  ...
  if (PARAMS.TradeWithContract === true) {
    var ttx = await _buybot.populateTransaction.swapExactETHForTokens(
      _router,
      _purchaseTokenAddress,
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (PARAMS.maxWalletBuy == true) {
        ...
        Log('Max wallet limit: ', maxWallet.toString(), `|| ${ethers.utils.formatUnits(maxWallet.toString(), TOKEN_DECIMAL)}`)
        const currentBalance = await TOKEN.balanceOf(_wallet.address);
        Log('Current wallet balance: ', currentBalance.toString(), `|| ${ethers.utils.formatUnits(currentBalance.toString(), TOKEN_DECIMAL)}`)
        const amountToBuy = maxWallet - currentBalance;
        Log('amountToBuy: ', amountToBuy, `|| ${ethers.utils.formatUnits(BigInt(amountToBuy).toString(), TOKEN_DECIMAL)}`)
        const amountToBuyFormated = ethers.BigNumber.from(BigInt(amountToBuy).toString())
        const walletBalance = await _wallet.getBalance();
        const formatedEther = ethers.utils.formatEther(walletBalance);
        const precision = 12;
        const divisor = ethers.BigNumber.from('10').pow(precision);
        ...
        ttx = await _uniRouter.populateTransaction.swapETHForExactTokens(
          truncatedValue
            ? `${truncatedValue
          ...
          { value: ethers.utils.parseEther((formatedEther - 0.06).toString()) }
        );
        ...
        if (_realBuyMethod == 0) {
          ...
          // ttx =
          //   await _uniRouter.populateTransaction.swapExactTokensForTokensSupportingFeeOnTransferTokens(
          //     _wethSellAmount,
          //     1,
          //     path,
          //     _wallet.address,
          ...
          if (_wethSellAmount !== 0) {
            ...
            const ao = amountsOut[path.length - 1];
            ttx =
              await _uniRouter.populateTransaction.swapExactETHForTokensSupportingFeeOnTransferTokens(
          ...
          const parsedNumber = ethers.utils.parseEther(
            `${randNumber.toFixed(10)}`
          );
          ...
          const walletBalance = await _wallet.getBalance();
          var formatedEther = ethers.utils.formatEther(walletBalance);
          if (formatedEther > 0.005) {
            ...
            const ao = ethers.utils.parseEther(
              parseFloat(
                ethers.utils.formatEther(amountsIn[0].mul(2)).toString()
              ).toFixed(18)
            );
            ttx = await _uniRouter.populateTransaction.swapETHForExactTokens(
              parsedNumber
...
const generateRugPullTx = async (
  ...
  var tx = [];
  if (PARAMS.TradeWithContract === true) {
    var _ttx = await _buybot.populateTransaction.swapExactTokensForETH(
      _router,
      _purchaseTokenAddress,
      _liquidityTokenAddress,
      _recipients,
    ...
    const TOKEN = new ethers.Contract(_purchaseTokenAddress, ERC20_ABI, wallet);
    var path = [];
    if (WETH.address === _liquidityTokenAddress) {
      path = [_purchaseTokenAddress, WETH.address];
    } else {
      path = [_purchaseTokenAddress, _liquidityTokenAddress, WETH.address];
    }
    for (var i = 0; i < keys.length; i++) {
      var _wallet = new ethers.Wallet(keys[i], wallet.provider);
      ...
      if (_params[0] > 0) {
        sellAmount = balance.mul(_params[0]).div(ethers.utils.parseEther("1"));
      } else {
      ...
      if (parseFloat(sellAmount.toString()) !== 0) {
        try {
          ...
          var _ttx =
            ...
            await _uniRouter.populateTransaction.swapExactTokensForETHSupportingFeeOnTransferTokens(
              sellAmount,
...
async function getMaxTokenAmount(TOKEN) {
  ...
  for (const limitFunction of possibleFunctions) {
    try {
      ...
      console.log(`Function "${limitFunction}" found. Max token amount:`, maxWallet.toString());
